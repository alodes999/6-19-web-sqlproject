...
This all does work, with a caveat.  It doesn't return the id number, it returns the name associated with it, so it jacks tables up.  Have to check on how to return the id itself, so that the drop downs are more expressive and I can remove the ul of types/locations.